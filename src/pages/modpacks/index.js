...
export function getStaticProps() {
    const allPostsData = getSortedPostsData('modpacks');
    return {
        props: {
            allPostsData,
        },
    ...
    return (
        <div>
            ...
            <main className='min-h-screen'>
                <Navbar />
                <p className='text-white mt-5 text-center text-3xl font-bold'>Modpacks</p>
                    ...
                    {props.allPostsData.map((post) => (
                        <article className='grid grid-cols-3 text-white border rounded-2xl w-96 sm:w-[36rem] md:w-[48rem] bg-zinc-800 border-zinc-900 p-5 '>
                            ...
                            <div className='flex flex-col'>
                                <Link className=' font-bold text-xl line-clamp-2' href={`/modpacks/${post.id}`}> {post.title} </Link>
